...
def create_hik(reference=None, name="Character1",suffix='suffix'):
    if reference is None:
        ...
        if not len(selected) == 1:
            lprint(u"错误: 未选择任何对象或选择了多个对象")
            return None
        reference = selected[0]
    lprint (u"创建HIK角色，参考对象:", reference, reference.name())
    if reference.name().endswith(suffix):
        ...
        lprint(u"使用suffix模式，找到的关节:", joints.keys())
    else:
        # Root_M
        ...
        lprint(u"使用Root_M模式，找到的关节:", joints.keys())

    char = pm.mel.hikCreateCharacter(name)
    lprint(u"创建的HIK角色:", char)
    if not char:
        lprint(u"错误: HIK角色创建失败")
        return None
    ...
    for i in range(1, 212):
        ...
        assigned_count += 1
    lprint(u"成功分配的关节数量:", assigned_count)
    if assigned_count == 0:
        lprint(u"警告: 没有关节被分配到HIK角色")
    return char
...
def get_cons(prefix="", suffix="", names=None):
    """
    names里面的名字name，如果prefix + name + suffix名字唯一性
    就添加物体进数组，返回这个数组
    """
    if names is None:
        return []
...
def find_cons(prefix=""):
    # 寻找跟骨?,腿部,脊柱,手臂和脖子的骨骼,和手指骨?
    lprint("正在查找控制器，使用prefix:", prefix)
    ...
    # 添加调试信息
    lprint("找到的控制器数量: root=", 1 if root else 0, ", l_leg=", len(l_leg), ", r_leg=", len(r_leg), ", spline=", len(spline), ", l_arm=", len(l_arm), ", r_arm=", len(r_arm))
    ...
    # 安全检查neck列表是否为空，避免pop操作失败
    if neck:
        spline.append(neck.pop(0))
    else:
        lprint("警告: 未找到Chest/Neck/Head控制器，使用prefix:", prefix)
        # 如果没有找到neck控制器，尝试查找其他可能的命名方式
        alternative_neck = get_cons(prefix+"FK", "_M", ["Chest1", "Neck1", "Head1"])
        if alternative_neck:
            spline.append(alternative_neck.pop(0))
            neck = alternative_neck
        else:
            lprint("错误: 完全未找到neck相关控制器")
...
def reference_fbx(sender):# 传递动捕数据
    ...
    if not con.isReferenced():
        QMessageBox.information(None, '消息提示!',
                                "小哥哥,你选择的文件需要是引用的形式额")
        return
    # 恢复HIK.py文件中选择动捕文件的功能（使用下拉列表）
    path= tranDoBuDataUI_Instance.fbxSelWidgets[0].currentText()

    prefix = con.namespace() # 获取Tpose节点的名称空间
    ...
    print("locator_node", locator_node)
    refNodeNameSpace=locator_node.namespace()[:-1]
    
    # 安全获取根节点名称，确保返回字符串而不是PyMel对象
    try:
        refNodeRootNode_obj = locator_node.root()
        if refNodeRootNode_obj:
            refNodeRootNode = str(refNodeRootNode_obj.name()) if hasattr(refNodeRootNode_obj, 'name') else str(refNodeRootNode_obj)
        else:
            refNodeRootNode = str(locator_node.name()).split(':')[0] + ':' + str(locator_node.name()).split(':')[1].split('Shape')[0]
    except Exception as e:
        # 如果root()方法失败，尝试从locator节点名称推断根节点名称
        lprint("获取根节点失败，使用备用方法:", str(e))
        locator_name = str(locator_node.name())
        refNodeRootNode = locator_name.replace('Shape', '')
    
    lprint("refNodeRootNode", refNodeRootNode)
    ...
    if not rig:
        lprint(u"错误: HIK角色创建失败，无法继续")
        return
    lprint(u"HIK角色创建完成:", rig)
...
class tranDoBuDataUI(QWidget):
    def __init__(self, parent = None):
        ...
        # 添加执行rename.mel脚本的按钮作为第一个功能
        widgetList=[(QPushButton,'',{'setText':u"执行骨骼重命"})]
        self.RenameBones=PySideLib.LQHVGrp(widgetList=widgetList,par=self.topLay,groupBox=u'1.执行骨骼重命')
        self.RenameBones.get_widgetList()[0].clicked.connect(self.executeRenameScript)
        ...
        self.SetSelTpose_widget=PySideLib.LQHVGrp(widgetList=widgetList,par=self.topLay,groupBox=u'2,请在大纲视图中选择Tpose任意一节点,点击设置选择')
        ...
        # 恢复HIK.py文件中选择动捕文件的UI和功能（使用LPathSel组件和路径选择方式）
        self.fbxSelWidget=PySideLib.LPathSel(par=self.topLay,l_lab='',buttonName=u'选择动捕文件',DialogCommit='choose fbx',fileType='*.fbx',chooseFunc='getOpenFileName',defaultPath='.fbx',groupBox=u'3,选择动捕文件')
        self.fbxSelWidgets=self.fbxSelWidget.get_widgetList()
        self.fbxSelWidget.setText(r"H:\JZXCL\5.Anima final\DB\mocap_30f\ep001\SC001_001_SHOT1-20_001\SC001_001_SHOT1-20_001_30f_hanyang.fbx")
        
        # 隐藏当前文件"3请在大纲视图中选择动捕文件根节点击设置选择"的UI和功能（注释掉相关代码）
        '''
        # 修改动捕文件选择逻辑，从路径选择改为场景中选择对象
        widgetList=[(QLineEdit,'',{u'setText':u'请选择场景中的动捕文件根节点?','setReadOnly':1},(0,1,1,3)),
                    (QPushButton,'',{u'setText':u'设置选择'})]
        self.fbxSelWidget=PySideLib.LQHVGrp(widgetList=widgetList,par=self.topLay,groupBox=u'3,请在大纲视图中选择动捕文件根节点?,点击设置选择')
        self.fbxSelWidgets=self.fbxSelWidget.get_widgetList()
        self.fbxSelWidgets[1].clicked.connect(self.SetFbxNodeFunc)
        '''
        
        StepThreeGB=QGroupBox(u'4.传递动捕数据(设置好帧速率命令才可用)')
    ...
    def postProcess(self):
        ...
        self.SetCtlPar_widget=PySideLib.LQHVGrp(widgetList=widgetList,par=self.topLay,groupBox=u'4.设置大臂和头部的控制器global参数?10')
        ...
        # frameRangeLay.addWidget(QLabel(u'帧范?:'))
        ...
        self.bakeCtl_widget=PySideLib.LQHVGrp(widgetList=widgetList,par=self.topLay,groupBox=u'6.烘焙动补数据到控制器')
        self.bakeCtl_widget.get_widgetList()[0].clicked.connect(self.bakeCtlFunc)




        widgetList=[(QPushButton,'',{'setText':u"移除动捕Fbx文件及临时数?"})]
        self.RemoveData=PySideLib.LQHVGrp(widgetList=widgetList,par=self.topLay,groupBox=u'7.移除动捕Fbx文件及临时文件')
    ...
    # 移除设置动捕文件节点选择的函数（因为不再需要）
    '''
    # 添加设置动捕文件节点选择的函数
    def SetFbxNodeFunc(self):
        selected = cmds.ls(sl=1)
        if not selected:
            QMessageBox.information(None, '消息提示!', "请选择场景中的动捕文件根节点")
            return
        # 设置选择的动捕文件节点
        self.fbxSelWidgets[0].setText(selected[0])
        
        # 移除对动捕文件必须是引用形式的限制
        # 检查选择的对象是否是引用对象
        # if not cmds.referenceQuery(selected[0], isNodeReferenced=True):
        #     QMessageBox.information(None, '消息提示!', "选择的文件需要是引用的形式"
        #     return
            
        # 获取文件路径（支持引用和非引用形式）
        if cmds.referenceQuery(selected[0], isNodeReferenced=True):
            fbxPath = cmds.referenceQuery(selected[0], filename=True)
        else:
            # 如果不是引用形式，获取场景文件路径
            fbxPath = "当前场景文件（非引用形式）"
        lprint("动捕文件路径:", fbxPath)
    '''
    ...
    def executeRenameScript(self):
        """执行rename.mel脚本进行骨骼重命?"""
        try:
            # 获取脚本文件的完整路?
            script_path = os.path.join(os.path.dirname(__file__), '__rename.mel')
            
            # 检查文件是否存?
            if not os.path.exists(script_path):
                QMessageBox.information(None, u'错误提示', u"未找到rename.mel文件: " + script_path)
                return
                
            # 执行MEL脚本，使用cmds模块
            mel_command = u'source "{}";renameBonesToNamespace();'.format(script_path.replace('\\', '/'))
            lprint( mel_command)
            mm.eval(mel_command)
