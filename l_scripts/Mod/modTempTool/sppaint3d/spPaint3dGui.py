...
class spPaint3dWin (object):
    ...
    def __init__(self):
        ...
        self.uiWin = mc.window(spPaint3dGuiID, title=(u"布景工具" + str(spPaint3dVersion)), width=493, resizeToFitChildren=True, sizeable=True, titleBar=True, minimizeButton=False, maximizeButton=False, menuBar=False, menuBarVisible=False, toolbox=True)
        ...
        self.uiBtnHelp = mc.button(label=u'帮助', command=lambda * args:self.uiButtonCallback("uiBtnHelp", args))
        self.uiBtnOptions = mc.button(label=u'属性', command=lambda * args:self.uiButtonCallback("uiBtnOptions", args))
        ...
        # Source Frame
        #----------------------
        self.uiSourceFrame = mc.frameLayout(label=u'绘制元素', cll=True, collapseCommand=lambda:self.resizeWindow('collapse', 98), expandCommand=lambda:self.resizeWindow('expand', 98), mh=5, mw=5, )
        ...
        #self.uiSourceBtnAdd = mc.symbolButton(w=60, h=18, ann=u'将选择物体添加进列表', image='{}/sp3dadd.xpm'.format(sp3d_icon_dir), command=lambda * args:self.uiListCallback("add", "uiSourceList"))
        #self.uiSourceBtnRem = mc.symbolButton(w=60, h=18, ann=u'从列表中移除选择物体', image='{}/sp3drem.xpm'.format(sp3d_icon_dir), command=lambda * args:self.uiListCallback("rem", "uiSourceList"))
        #self.uiSourceBtnClr = mc.symbolButton(w=60, h=18, ann=u'清空列表', image='{}/sp3dclr.xpm'.format(sp3d_icon_dir), command=lambda * args:self.uiListCallback("clr", "uiSourceList"))
        self.uiSourceBtnAdd = mc.button(w=60, h=18, l=u"+ 添加", ann=u'将选择物体添加进列表', command=lambda *args: self.uiListCallback("add", "uiSourceList"))
        self.uiSourceBtnRem = mc.button(w=60, h=18, l=u"- 移除", ann=u'从列表中移除选择物体', command=lambda * args:self.uiListCallback("rem", "uiSourceList"))
        self.uiSourceBtnClr = mc.button(w=60, h=18, l=u"c 清空", ann=u'清空列表', command=lambda * args:self.uiListCallback("clr", "uiSourceList"))

        mc.formLayout(self.uiSourceForm, edit=True,
                      attachForm=[(self.uiSourceList, 'top', 0),
                                  (self.uiSourceList, 'left', 0),
                                  (self.uiSourceList, 'right', 0),
                                  (self.uiSourceBtnAdd, 'left', 0),
                                  (self.uiSourceBtnClr, 'right', 0)],
                      ...
                      ap=[(self.uiSourceBtnAdd, 'right', 3, 33),
                          (self.uiSourceBtnRem, 'right', 3, 66)])
        ...
        # Transform Setup
        #----------------------
        self.uiTransformFrame = mc.frameLayout(label=u'随机属性设置', cll=True, collapseCommand=lambda:self.resizeWindow('collapse', 177), expandCommand=lambda:self.resizeWindow('expand', 177), mh=5, mw=5, )
        self.uiTransformForm = mc.formLayout(numberOfDivisions=100, width=255)
        self.uiTransformRotateCheck = mc.checkBox(label=u'旋转', ann=u'绘制时应用旋转属性设置', changeCommand=lambda * args:self.uiCheckBoxCallback("transformRotate", args))
        self.uiTransformRotateFieldX = mc.floatFieldGrp(numberOfFields=2, label=u'最小', bgc=(0.8, 0.3, 0.3 ), extraLabel=u'最大', cw4=(22, 50, 50, 30), precision=2, ct4=('right', 'both', 'both', 'right'), co4=(2, 2, 2, 8), changeCommand=lambda * args:self.uiTransformCallback())
        self.uiTransformRotateFieldY = mc.floatFieldGrp(numberOfFields=2, label='', bgc=(0.3, 0.8, 0.3), extraLabel='', cw4=(22, 50, 50, 30), precision=2, ct4=('right', 'both', 'both', 'right'), co4=(2, 2, 2, 8), changeCommand=lambda * args:self.uiTransformCallback())
        self.uiTransformRotateFieldZ = mc.floatFieldGrp(numberOfFields=2, label='', bgc=(0.3, 0.3, 0.8), extraLabel='', cw4=(22, 50, 50, 30), precision=2, ct4=('right', 'both', 'both', 'right'), co4=(2, 2, 2, 8), changeCommand=lambda * args:self.uiTransformCallback())
        self.uiTransformSeparator = mc.separator(height=5, width=250, style='in')
        self.uiTransformScaleCheck = mc.checkBox(label=u'缩放', ann=u'绘制时应用缩放属性设置', changeCommand=lambda * args:self.uiCheckBoxCallback("transformScale", args))
        self.uiTransformScaleUniformCheck = mc.checkBox(label=u'统一', ann=u'yz轴属性应用x轴缩放数值，(勾选时只需设置x轴的缩放范围)', changeCommand=lambda * args:self.uiCheckBoxCallback("transformScaleUniform", args))
        self.uiTransformScaleFieldX = mc.floatFieldGrp(numberOfFields=2, label=u'最小', backgroundColor=(0.8, 0.3, 0.3 ), extraLabel=u'最大', cw4=(22, 50, 50, 30), precision=2, ct4=('right', 'both', 'both', 'right'), co4=(2, 2, 2, 8), v1=1.0, v2=1.0, changeCommand=lambda * args:self.uiTransformCallback())
        self.uiTransformScaleFieldY = mc.floatFieldGrp(numberOfFields=2, label='', backgroundColor=(0.3, 0.8, 0.3), extraLabel='', cw4=(22, 50, 50, 30), precision=2, ct4=('right', 'both', 'both', 'right'), co4=(2, 2, 2, 8), v1=1.0, v2=1.0, changeCommand=lambda * args:self.uiTransformCallback())
        self.uiTransformScaleFieldZ = mc.floatFieldGrp(numberOfFields=2, label='', backgroundColor=(0.3, 0.3, 0.8), extraLabel='', cw4=(22, 50, 50, 30), precision=2, ct4=('right', 'both', 'both', 'right'), co4=(2, 2, 2, 8), v1=1.0, v2=1.0, changeCommand=lambda * args:self.uiTransformCallback())
        
        mc.formLayout(self.uiTransformForm, edit=True,
            attachForm=[(self.uiTransformRotateCheck, 'top', 4),
                        (self.uiTransformRotateFieldX, 'top', 0),
                        (self.uiTransformRotateFieldX, 'right', 4),
                        (self.uiTransformRotateFieldY, 'right', 4),
                        (self.uiTransformRotateFieldZ, 'right', 4),
                        (self.uiTransformSeparator, 'right', 4),
                        (self.uiTransformScaleFieldX, 'right', 4),
                        (self.uiTransformScaleFieldY, 'right', 4),
                        (self.uiTransformScaleFieldZ, 'right', 4),
                        ],
        ...
        # Target Surface(s)
        #----------------------
        self.uiTargetFrame = mc.frameLayout(label=u'绘制目标（地面或山体模型）', cll=True, collapseCommand=lambda:self.resizeWindow('collapse', 98), expandCommand=lambda:self.resizeWindow('expand', 98), mh=5, mw=5, )
        ...
        # self.uiTargetBtnAdd = mc.symbolButton(w=60, h=18, ann=u'将选择物体添加进列表', image='{}/sp3dadd.xpm'.format(sp3d_icon_dir), command=lambda * args:self.uiListCallback("add", "uiTargetList"))
        # self.uiTargetBtnRem = mc.symbolButton(w=60, h=18, ann=u'从列表移除所选物体', image='{}/sp3drem.xpm'.format(sp3d_icon_dir), command=lambda * args:self.uiListCallback("rem", "uiTargetList"))
        # self.uiTargetBtnClr = mc.symbolButton(w=60, h=18, ann=u'清空列表', image='{}/sp3dclr.xpm'.format(sp3d_icon_dir), command=lambda * args:self.uiListCallback("clr", "uiTargetList"))
        self.uiTargetBtnAdd = mc.button(w=60, h=18, l=u"+ 添加", ann=u'将选择物体添加进列表', command=lambda *args: self.uiListCallback("add", "uiTargetList"))
        self.uiTargetBtnRem = mc.button(w=60, h=18, l=u"- 移除", ann=u'从列表中移除选择物体', command=lambda *args: self.uiListCallback("rem", "uiTargetList"))
        self.uiTargetBtnClr = mc.button(w=60, h=18, l=u"c 清空", ann=u'清空列表', command=lambda *args: self.uiListCallback("clr", "uiTargetList"))
        # mc.formLayout(self.uiTargetForm, edit=True,
        ...
        mc.formLayout(self.uiTargetForm, edit=True,
                      attachForm=[(self.uiTargetList, 'top', 0),
                                  (self.uiTargetList, 'left', 0),
                                  (self.uiTargetList, 'right', 0),
                                  (self.uiTargetBtnAdd, 'left', 0),
                                  (self.uiTargetBtnClr, 'right', 0)],
                      attachControl=[(self.uiTargetBtnAdd, 'top', 3, self.uiTargetList),
                                     (self.uiTargetBtnRem, 'left', 5, self.uiTargetBtnAdd),
                                     (self.uiTargetBtnRem, 'top', 3, self.uiTargetList),
                                     (self.uiTargetBtnClr, 'left', 5, self.uiTargetBtnRem),
                                     (self.uiTargetBtnClr, 'top', 3, self.uiTargetList)],
                      ap=[(self.uiTargetBtnAdd, 'right', 3, 33),
                          (self.uiTargetBtnRem, 'right', 3, 66)])
        mc.setParent(self.uiTopColumn)
        
        #----------------------
        # Paint Contexts
        #----------------------
        self.uiPaintFrame = mc.frameLayout(label=u'绘制', cll=True, collapseCommand=lambda:self.resizeWindow('collapse', 61), expandCommand=lambda:self.resizeWindow('expand', 61), mh=5, mw=5, )
        self.uiPaintForm = mc.formLayout(numberOfDivisions=100, width=255)
        # self.uiPaintDupSCB = mc.symbolCheckBox(w=52, h=18, ann=u'复制方式切换: 关联\复制', ofi='{}/sp3dduplicate.xpm'.format(sp3d_icon_dir), oni='{}/sp3dinstance.xpm'.format(sp3d_icon_dir), changeCommand=lambda * args:self.uiCheckBoxCallback("instance", args))
        # self.uiPaintRandSCB = mc.symbolCheckBox(w=52, h=18, ann=u'分布方式: 随机\连续', ofi='{}/sp3dsequence.xpm'.format(sp3d_icon_dir), oni='{}/sp3drandom.xpm'.format(sp3d_icon_dir), changeCommand=lambda * args:self.uiCheckBoxCallback("random", args))
        # self.uiPaintAlignSCB = mc.symbolCheckBox(w=100, h=18, ann=u'对齐到平面法线', ofi='{}/sp3dalignoff.xpm'.format(sp3d_icon_dir), oni='{}/sp3dalign.xpm'.format(sp3d_icon_dir), changeCommand=lambda * args:self.uiCheckBoxCallback("align", args))
        # self.uiPaintCtxBtn = mc.symbolButton(w=105, h=28, ann=u'绘制', image='{}/sp3dpaint.xpm'.format(sp3d_icon_dir), command=lambda * args:self.genericContextCallback("PaintCtx"))
        # self.uiPlaceCtxBtn = mc.symbolButton(w=105, h=28, ann=u'摆放', image='{}/sp3dplace.xpm'.format(sp3d_icon_dir), command=lambda * args:self.genericContextCallback("PlaceCtx"))
        self.uiPaintDupSCB = mc.checkBox(w=52, h=18, l=u"关联", ann=u'复制方式切换: 关联\复制', cc=lambda * args:self.uiCheckBoxCallback("instance", args))
        self.uiPaintRandSCB = mc.checkBox(w=52, h=18, l=u"随机", ann=u'分布方式: 随机\连续', changeCommand=lambda *args: self.uiCheckBoxCallback("random", args))
        self.uiPaintAlignSCB = mc.checkBox(w=100, h=18, l=u"对齐到法线", ann=u'对齐到平面法线', changeCommand=lambda *args: self.uiCheckBoxCallback("align", args))

        self.uiPaintCtxBtn = mc.button(w=105, h=28, l=u"绘制", ann=u'绘制', command=lambda * args:self.genericContextCallback("PaintCtx"))
        self.uiPlaceCtxBtn = mc.button(w=105, h=28, l=u"摆放", ann=u'摆放', command=lambda * args:self.genericContextCallback("PlaceCtx"))
        mc.formLayout(self.uiPaintForm, edit=True,
                      attachForm=[(self.uiPaintDupSCB, 'top', 0),
                                  (self.uiPaintDupSCB, 'left', 0),
                                  (self.uiPaintAlignSCB, 'right', 0),
                                  (self.uiPaintCtxBtn, 'left', 0),
                                  (self.uiPlaceCtxBtn, 'right', 0),],
                      ...
                      ap=[(self.uiPaintDupSCB, 'right', 3, 33),
                          (self.uiPaintRandSCB, 'right', 3, 66),
                          (self.uiPaintCtxBtn, 'right', 3, 50),])
        ...
        # Paint Metrics
        #----------------------
        self.uiPaintMetricsFrame = mc.frameLayout(label=u'绘制属性设置', cll=True, collapseCommand=lambda:self.resizeWindow('collapse', 129), expandCommand=lambda:self.resizeWindow('expand', 129), mh=5, mw=5, )
        self.uiPaintMetricsForm = mc.formLayout(numberOfDivisions=100, width=255)

        self.uiPaintTimer = mc.floatSliderGrp(label=u'灵敏度', field=1, minValue=0.0, maxValue=0.2, fieldMaxValue=1.0, precision=2, vis=False, w=250, cw=[(1, 55), (2, 35)], changeCommand=lambda * args:self.uiFluxCallback("paintTimer", args))
        self.uiPaintDistance = mc.floatFieldGrp(label=u'绘制间隔（数值越小越密集）', precision=2, vis=False, w=350, cw=[(1, 150),(2, 200)], changeCommand=lambda * args:self.uiFluxCallback("paintDistance", args))
        self.uiPaintMetricsSep1 = mc.separator(w=250)
        self.uiUpOffset = mc.floatFieldGrp(label=u'高度偏移', precision=2, vis=True, w=100, cw=[(1, 54), (2, 40)], changeCommand=lambda * args:self.uiPaintOffsetCallback("upOffset", args))
        self.uiPaintMetricsRampMenu = mc.optionMenu(l=u'梯度影响', changeCommand=lambda * args:self.uiRampMenuCallback("rampMenu", args))
        mc.menuItem(label=' ')
        mc.menuItem(label=u'旋转')
        mc.menuItem(label=u'缩放')
        mc.menuItem(label=u'旋转|缩放')
        self.uiPaintMetricsSep2 = mc.separator(w=250)


        self.uiJitterCheck = mc.checkBox(label=u'抖动偏移', ann=u'在绘制位置上进行UV方向的位置偏移', changeCommand=lambda * args:self.uiCheckBoxCallback("jitter", args))
        self.uiJitterFieldU = mc.floatFieldGrp(numberOfFields=2, label=u'U最小', backgroundColor=(.895, .735, 0.176), extraLabel=u'最大', cw4=(32, 50, 50, 30), precision=2, ct4=('right', 'both', 'both', 'right'), co4=(2, 2, 2, 8), changeCommand=lambda * args:self.uiTransformCallback())
        self.uiJitterFieldV = mc.floatFieldGrp(numberOfFields=2, label=u'V最小', backgroundColor=(.692, .323, 0.851), extraLabel=u'最大', cw4=(32, 50, 50, 30), precision=2, ct4=('right', 'both', 'both', 'right'), co4=(2, 2, 2, 8), changeCommand=lambda * args:self.uiTransformCallback())
        ...
        self.Splatter = mc.checkBox(label=u'集束', w=50, ann=u'启用集束，绘制的元素将成簇状出现', changeCommand=lambda * args:self.uiRadiusCallback())
        self.radius = mc.intSliderGrp(field=True, label=u'半径', minValue=0, maxValue=300, fieldMinValue=0,
                                      fieldMaxValue=100000, value=30, changeCommand=lambda * args:self.uiRadiusCallback())

        self.numRange = mc.intFieldGrp(numberOfFields=2, label=u'最小', extraLabel=u'最大', value1=1, value2=10,
        ...
        #                              ])
        mc.formLayout(self.uiPaintMetricsForm, edit=True,
                      attachForm=[(self.uiPaintTimer, 'top', 0),
                                  (self.uiPaintDistance, 'top', 0),
                                  (self.uiPaintDistance, 'left', 0),
                                  (self.uiPaintDistance, 'right', 0),
                                  (self.uiPaintMetricsSep1, 'left', 0),
                                  (self.uiPaintMetricsSep1, 'right', 0),
                                  (self.uiPaintMetricsRampMenu, 'right', 0),
                                  (self.uiPaintMetricsSep2, 'left', 0),
                                  (self.uiPaintMetricsSep2, 'right', 0),
                                  (self.uiJitterFieldU, 'right', 0),
                                  (self.uiJitterFieldV, 'right', 0),
                                  # (self.uiPaintMetricsRampMenu, 'right', 0),
                                  (self.uiUpOffset, 'left', 0),
                                  (self.uiPaintMetricsSep3, 'left', 0),
                                  (self.uiPaintMetricsSep3, 'right', 0),
                                  (self.Splatter, 'left', 0),
                                  (self.radius, 'left', 50),
                                  (self.radius, 'right', 0),
                                  (self.numRange, 'left', 50),
                                  (self.numRange, 'right', 0), ],
                      attachControl=[(self.uiPaintMetricsSep1, 'top', 5, self.uiPaintTimer),
                                     ...
                                     (self.uiJitterFieldV, 'top', 5, self.uiJitterFieldU),
                                     (self.uiJitterFieldV, 'left', 5, self.uiJitterCheck),
                                     (self.uiPaintMetricsSep3, 'top', 5, self.uiJitterFieldV),
                                     (self.Splatter, 'top', 5, self.uiPaintMetricsSep3),
                                     (self.Splatter, 'right', 5, self.radius),
                                     (self.radius, 'top', 5, self.uiPaintMetricsSep3),
                                     (self.numRange, 'top', 5, self.Splatter), ],
                      ap=[(self.uiUpOffset, 'right', 3, 50), ])
        mc.setParent(self.uiTopColumn)
        ...
        self.errorHandle = sp3derror((spPaint3dGuiID + ' ' + str(spPaint3dVersion) + ' | czthach_2019'), self.uiInfoTextField)
    ...
    def uiButtonCallback(self, *args):
        ...
        button = args[0]
        if (button == 'uiBtnHelp'):
            mc.confirmDialog(title=spPaint3dGuiID + ' ' + str(spPaint3dVersion) + u' 帮助', message=u'有关如何使用脚本的详细帮助，请参阅公司wiki帮助：\nhttp://192.168.100.18/pages/viewpage.action?pageId=2097229\n或者使用选项中的主页按钮。', button=u'继续');
    ...
    def updateUIControls(self, ui):
        ...
        mc.checkBox(self.uiPaintRandSCB, edit=True, value=ui.random)
        mc.checkBox(self.uiPaintAlignSCB, edit=True, value=ui.align)
    ...
    def genericContextCallback(self, *args):
        ...
        if(not sourcevalid):
            self.errorHandle.raiseError(u"绘制元素列表为空，或列表中的元素不存在.");
        elif(not targetvalid):
            self.errorHandle.raiseError(u"绘制目标列表为空，或列表中的元素不存在.");
        elif(duplicateerror):
            self.errorHandle.raiseError(u"选中物体不能同时添加到两份列表当中!");
        else:
            ...
            if (args[0] == 'PaintCtx'):
                ...
                self.errorHandle.raiseError(u"执行！全力绘制……")
                ...
                self.errorHandle.raiseError(u"执行！全力摆放……")
    ...
    def setupWin(self, uiOptions):
        ...
        Create setup UI
        '''
        self.uiSetupWin = mc.window(spPaint3dSetupID, title=(u"布景工具" + str(spPaint3dVersion)), width=250, height=450, resizeToFitChildren=True, sizeable=True, titleBar=True, minimizeButton=False, maximizeButton=False, menuBar=False, menuBarVisible=False, toolbox=True)
        ...
        self.uiSetupBtnHelp = mc.button(label=u'帮助', command=lambda * args:self.setupButtonCallback('uiSetupBtnHelp', args))
        self.uiSetupBtnHomepage = mc.button(label=u'主页', command=lambda * args:self.setupButtonCallback('uiSetupBtnHomepage', args))
        self.uiSetupBtnReset = mc.button(label=u'重置', command=lambda * args:self.setupButtonCallback('uiSetupBtnReset', args))
        ...
        # Duplicate Options
        #----------------------
        self.uiSetupDuplicateFrame = mc.frameLayout(label=u'复制属性',  marginHeight=5, marginWidth=20)
        self.uiSetupDuplicateForm = mc.formLayout(numberOfDivisions=100)
        self.uiSetupChkInputConn = mc.checkBoxGrp(label=u'保持输入连接', changeCommand=lambda * args:self.setupCallback('uiSetupChkInputConn', args), numberOfCheckBoxes=1, width=170)
        ...
        # Surface Normals
        #----------------------
        self.uiSetupNormalFrame = mc.frameLayout(label=u'曲面法线',  marginHeight=5, marginWidth=20)
        self.uiSetupNormalForm = mc.formLayout(numberOfDivisions=100)
        self.uiSetupNormalCol = mc.radioCollection()
        self.uiSetupNormalSmooth = mc.radioButton(label=u'几何体法线', align='right', onCommand=lambda * args:self.setupCallback('uiSetupNormalCol', True))
        self.uiSetupNormalHard = mc.radioButton(label=u'强制硬表面法线', align='right', onCommand=lambda * args:self.setupCallback('uiSetupNormalCol', False))
        ...
        # Flux control
        #----------------------
        self.uiSetupFluxFrame = mc.frameLayout(label=u'绘制控制',  marginHeight=5, marginWidth=20)
        self.uiSetupFluxForm = mc.formLayout(numberOfDivisions=100)
        self.uiSetupFluxCol = mc.radioCollection()
        self.uiSetupFluxTimer = mc.radioButton(label=u'计时', align='right', onCommand=lambda * args:self.setupCallback('uiSetupFluxCol', False))
        self.uiSetupFluxDistance = mc.radioButton(label=u'绘制间距', align='right', onCommand=lambda * args:self.setupCallback('uiSetupFluxCol', True))
        ...
        # Hierarchy
        #----------------------
        self.uiSetupHierarchyFrame = mc.frameLayout(label=u'层级管理',  marginHeight=5, marginWidth=20)
        self.uiSetupHierarchyForm = mc.formLayout(numberOfDivisions=100)
        self.uiSetupHierarchyActive = mc.checkBoxGrp(label=u'启用元素分组', changeCommand=lambda * args:self.setupCallback('uiSetupHierarchyActive', args), numberOfCheckBoxes=1)
        self.uiSetupHierarchyCol = mc.radioCollection()
        self.uiSetupHierarchySession = mc.radioButton(label=u'生成顺序单组', align='right', onCommand=lambda * args:self.setupCallback('uiSetupHierarchySession', args))
        self.uiSetupHierarchyStroke = mc.radioButton(label=u'按笔画分组', align='right', onCommand=lambda * args:self.setupCallback('uiSetupHierarchyStroke', args))
        self.uiSetupHierarchySource = mc.radioButton(label=u'按元素分组', align='right', onCommand=lambda * args:self.setupCallback('uiSetupHierarchySource', args))
        ...
        # Place Options
        #----------------------
        self.uiSetupPaintOptionsFrame = mc.frameLayout(label=u'摆放属性',  marginHeight=5, marginWidth=20)
        self.uiSetupPaintOptionsForm = mc.formLayout(numberOfDivisions=100)
        self.uiSetupPlaceRotate = mc.floatFieldGrp(label=u'旋转增量', precision=2, vis=True, w=250, cw=[(1, 145), (2, 50)], changeCommand=lambda * args:self.uiSetupPlaceRotateCallback("placeRotate", args))
        self.uiSetupContinuousTransform = mc.checkBoxGrp(label=u'连续变换', changeCommand=lambda * args:self.setupCallback('uiSetupContinuousTransform', args), numberOfCheckBoxes=1)
        ...
        # Dev feature
        #----------------------
        self.uiSetupDevFrame = mc.frameLayout(label=u'演化特征',  marginHeight=5, marginWidth=20)
        self.uiSetupDevForm = mc.formLayout(numberOfDivisions=100)
        self.uiSetupRealTimeRampFX = mc.checkBoxGrp(label=u'实时梯度影响', changeCommand=lambda * args:self.setupCallback('uiSetupRealTimeRampFX', args), numberOfCheckBoxes=1)
    ...
    def setupButtonCallback(self, *args):
        ...
        if(button == 'uiSetupBtnHelp'):
            mc.confirmDialog(title=spPaint3dGuiID + ' ' + str(spPaint3dVersion) + ' Help', message=u'有关如何使用脚本的详细帮助，请参阅公司wiki帮助：\nhttp://192.168.100.18/pages/viewpage.action?pageId=2097229\n或者使用选项中的主页按钮。', button=u'继续');
        elif(button == 'uiSetupBtnHomepage'):
            mc.showHelp('http://192.168.100.18/pages/viewpage.action?pageId=2097229', absolute=True);
...
def getMayaVersion():
    ...
    while not supportedversion:
        try:
            ...
            result = mc.promptDialog(title=u'输入Maya版本', message=u'不能判定maya版本，需要手动输入当前maya版本的四位数字(例如: 2017)', button=[u'确定', u'取消&退出'], defaultButton=u'确定', cancelButton=u'取消&退出', dismissString=u'取消&退出')
            if result == 'OK':
